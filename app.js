...
function guardarAnio() {
    ...
    try { localStorage.setItem("anioIngreso", anioIngreso); } catch(e) {}
    render();
}
...
function guardarEstado() {
    ...
    try { localStorage.setItem("estados", JSON.stringify(estados)); } catch(e) {}
    render();
}
...
function agregarProyectoExtension() {
    ...
    try { localStorage.setItem("proyectosExtension", JSON.stringify(proyectosExtension)); } catch(e) {}
...
// Check if first time
var hasSeenHelp;
try { hasSeenHelp = localStorage.getItem("hasSeenHelp"); } catch(e) { hasSeenHelp = null; }
if (!hasSeenHelp) {
    showHelpModal();
    try { localStorage.setItem("hasSeenHelp", "true"); } catch(e) {}
}
...
// Keyboard accessibility for toggle headers
